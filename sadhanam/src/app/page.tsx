...
async function getHomeData(userId: string | null) {
  ...
  // Build champs query (filter depends on userId)
  let champQ = supabase
    .from('championships')
  ...
  // ── Level 1: 4 independent top-level queries run in parallel ────────────────
  const [
    { data: rawLive },
    { data: rawChamps },
    { data: rawActive },
  ...
  // ── Derive ID lists for level-2 fan-out queries ──────────────────────────────
  const champIds    = (rawChamps  ?? []).map((c) => c.id)
  const activeEvIds = (rawActive  ?? []).map((e) => e.id)
  const recentEvIds = (rawRecent  ?? []).map((e) => e.id)

  // ── Level 2: 3 fan-out queries run in parallel ───────────────────────────────
  const [evRowsRes, activeMatchRes, finalsRes] = await Promise.all([
    champIds.length > 0
      ? supabase.from('tournaments').select('id, championship_id, status').in('championship_id', champIds)
      : Promise.resolve({ data: [] as { id: string; championship_id: string | null; status: string }[] }),
    activeEvIds.length > 0
      ? supabase.from('matches').select('tournament_id, status').in('tournament_id', activeEvIds)
  ...
  // ── After evRows resolve, we can fetch match counts for those events ──────────
  const evRows   = evRowsRes.data ?? []
  const allEvIds = evRows.map((e) => e.id)

  const champMatchRes = allEvIds.length > 0
    ? await supabase.from('matches').select('tournament_id, status').in('tournament_id', allEvIds)
    : { data: [] as { tournament_id: string; status: string }[] }

  // ── Build championship event/match counts ────────────────────────────────────
  ...
  if (champIds.length > 0) {
    ...
    for (const mr of champMatchRes.data ?? []) {
  ...
  for (const mr of (activeMatchRes.data ?? []) as { tournament_id: string; status: string }[]) {
