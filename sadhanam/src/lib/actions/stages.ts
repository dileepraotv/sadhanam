...
import type { MatchFormat, RRStageConfig, KOStageConfig, StageStatus, StageResetLog, TournamentFormatType } from '@/lib/types'
...
export async function revalidateTournamentPaths(
  ...
  tournamentId: string,
  /** Pass if already known — skips a DB round-trip */
  knownChampionshipId?: string | null,
) {
  // Only query when the caller hasn't already loaded championship_id
  let champId = knownChampionshipId
  if (champId === undefined) {
    const { data: t } = await supabase
      ...
      .eq('id', tournamentId)
      .single()
    champId = t?.championship_id ?? null
  }
...
export async function createRRStage(input: {
  ...
  if (input.numberOfGroups < 1) {
    return { error: 'Number of groups must be at least 1' }
  }
  ...
  // Insert rr_groups (Group 1, Group 2, …)
...
export async function stageHasScores(
  ...
  // Single query: count games whose match belongs to this stage via join.
  // Replaces the previous 2-step "fetch match IDs then count games" pattern.
  const { count } = await supabase
    .from('games')
    .select('match_id, matches!inner(stage_id)', { count: 'exact', head: true })
...
export async function getStageResetStats(stageId: string): Promise<{
  ...
  // Fetch matches and game count in parallel — they're independent reads
  const [matchRes, gameRes] = await Promise.all([
    supabase.from('matches').select('id, status').eq('stage_id', stageId),
    supabase
      .from('games')
      .select('match_id, matches!inner(stage_id)', { count: 'exact', head: true })
      .eq('matches.stage_id', stageId),
...
export async function getKOResetStats(tournamentId: string): Promise<{
  ...
  // Parallel: match statuses and game count are independent reads
  const [matchRes, gameRes] = await Promise.all([
  ...
  return { matchCount: matchRows.length, completedMatchCount, liveMatchCount, gameCount }
}
