...
export function MultiStagePanel({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {/* ── Top-level Next Step banner — consistent with SingleKO and SingleRR ── */}
      {rrPhase === 'no_stage' && players.length === 0 && (
        <NextStepBanner
          variant="info"
          step="Step 1 of 4"
          title="Add players first"
          description="Go to the Players tab and add at least 2 players before configuring stages."
        />
      )}
      {rrPhase === 'no_stage' && players.length >= 2 && (
        <NextStepBanner
          variant="action"
          step="Step 1 of 4"
          title={`${players.length} players ready — configure Stage 1 (Round Robin) below`}
          description="Set group size, advancement rules, then save the configuration."
        />
      )}
      {rrPhase === 'no_members' && (
        <NextStepBanner
      ...
      {rrPhase === 'no_fixtures' && (
        <NextStepBanner
          variant="action"
          step="Step 3 of 4"
          title="Players assigned — generate the Stage 1 schedule"
          description='Click "Generate RR Schedule" to create all round robin fixtures.'
        />
      )}
      {(rrPhase === 'in_progress' || rrPhase === 'all_complete') && !stage1Complete && (
        <NextStepBanner
          variant={rrPhase === 'all_complete' ? 'action' : 'warning'}
          step={rrPhase === 'all_complete' ? 'Step 3 complete' : undefined}
          title={rrPhase === 'all_complete'
            ? 'All Stage 1 matches done — finalize to advance qualifiers'
            : 'Stage 1 in progress — enter scores for each fixture'}
          description={rrPhase === 'all_complete'
            ? 'Use "Finalize Group Stage" in Stage 1 below to lock results and unlock Stage 2.'
            : 'Once all matches are complete, use Finalize to advance players to Stage 2.'}
        />
      )}
      {stage1Complete && !stage2Generated && (
        <NextStepBanner
          variant="action"
          step="Step 4 of 4"
          title="Stage 1 complete — generate the Stage 2 knockout draw"
          description='Click "Generate Knockout Groups" in Stage 2 below to create the final bracket.'
        />
      )}
      {stage2Generated && (
        <NextStepBanner
          variant="success"
          title="Both stages generated — manage Stage 2 matches below"
          description="Enter scores for knockout matches to determine the tournament winner."
        />
      )}
