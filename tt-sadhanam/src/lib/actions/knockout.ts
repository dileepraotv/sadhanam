...
export async function generateKnockoutStage(
  ...
  const [rrStageRes, groupRes, playerRes, matchRes] = await Promise.all([
    supabase.from('stages').select('config').eq('id', rrStageId).single(),
    supabase.from('rr_groups')
      .select('id, stage_id, name, group_number, rr_group_members(player_id)')
      .eq('stage_id', rrStageId).order('group_number'),
    ...
    supabase.from('matches')
      ...
      .eq('tournament_id', tournamentId).eq('stage_id', rrStageId),
  ])

  const rrStage   = rrStageRes.data
  ...
  // Bulk-upsert KO seeds — replaces N×serial updates with a single round-trip
  if (qualifiers.length > 0) {
    await supabase
      .from('players')
      .upsert(
        qualifiers.map(q => ({ id: q.playerId, seed: q.koSeed })),
  ...
  // Fetch max match_number while we have everything we need — no serial dependency
