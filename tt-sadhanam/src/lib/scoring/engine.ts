 ...
 * scoring UI. Always shows ALL possible game rows at once so scores can be
 * entered for every game in a single step (Save All workflow).
 *
 * Examples:
 *   Bo5, 0 games saved        → [1, 2, 3, 4, 5]  (all rows, all empty)
 *   Bo5, games 1–3 saved      → [1, 2, 3, 4, 5]  (all rows, 1-3 filled)
 *   Bo5, match complete 3–0   → [1, 2, 3]         (only through deciding game)
 */
export function inferGameNumbersToShow(
  ...
  if (state.outcome !== 'in_progress') {
    // Show only through the deciding game once the match is complete
