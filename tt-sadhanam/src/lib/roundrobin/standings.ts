 ...
 *
 * Convenience wrapper for a PURE ROUND ROBIN tournament (no groups).
 * Synthesises a single virtual group from all players, then delegates to
 * computeGroupStandings.
 *
 * Moved here from lib/actions/pureRoundRobin.ts so it can be called from
 * client components without violating the 'use server' restriction that
 * requires all exports in a server-actions file to be async functions.
 */
export function computeLeagueStandings(
  ...
  // Synthesise a single virtual group containing all players
  const virtualGroup: RRGroup = {
    id:          '__league__',
    stageId:     '__league__',
    name:        'League',
    groupNumber: 1,
  ...
  const rrMatches  = matches.filter(m => m.match_kind === 'round_robin' || !m.match_kind)
  const rrMatchIds = new Set(rrMatches.map(m => m.id))
  const rrGames    = games.filter(g => rrMatchIds.has(g.match_id))
  ...
  const result = computeGroupStandings(
    ...
    rrMatches,
    rrGames,
    1,   // advanceCount = 1 (winner of league)
  )
