...
export interface RRConfigValues {
  ...
  advanceCount:     number
  matchFormat:      MatchFormat   // always 'bo5' — set per-match by scorer, not here
...
export function RRConfigPanel({ players, onSubmit, isPending, existing, sport }: Props) {
  ...
  const [finalizeRule, setFinalizeRule] = useState<'require_all' | 'manual'>(
    existing?.finalizationRule ?? 'manual'   // default: manual override (admin controls when to advance)
  )

  const PPG    = Math.max(2, parseInt(perGroup) || 4)
  const layout = computeGroupLayout(players.length, PPG)
  // When a stage already exists, numberOfGroups is locked — use that directly.
  const G      = existing?.numberOfGroups ?? layout.numGroups
  const A      = parseInt(advanceCount) || 2
  const T      = allowThird ? (parseInt(thirdCount) || 2) : 0
  const totalQ = G * A + T

  // Valid as long as we have >= 2 players. The new algorithm guarantees
  // no group is ever smaller than PPG (>= 2), so no further min-size check needed.
  const validationError = existing ? null : validateGroupLayout(players.length, PPG, A)
  const canSubmit       = !existing && validationError === null
      ...
      <CardContent className="flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-[10px] text-muted-foreground font-medium text-orange-700 dark:text-orange-400">
              {players.length > 0
                ? <>→ {layoutLabel}</>
                : <>→ groups auto-calculated (add players first)</>
              }
            </p>
          </div>
        ...
        <div className={cn(
          ...
          <p className={cn(
            ...
            {canSubmit
              ? <>{layout.numGroups} group{layout.numGroups > 1 ? 's' : ''} · top {A} advance{allowThird ? ` + ${T} best-third` : ''} → <strong>{totalQ} total qualifiers</strong>.</>
              : <>{validationError}</>
            }
          </p>
          {/* Layout breakdown — always shown when valid and > 1 group */}
          {canSubmit && players.length > 0 && (
            <p className="text-xs mt-1 text-green-600 dark:text-green-500">
              ℹ {layoutLabel} — players snake-seeded across groups.
            </p>
          )}
        </div>
