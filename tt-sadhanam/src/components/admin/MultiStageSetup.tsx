...
export function MultiStageSetup({
  ...
  const handleCreateStage = () => {
    ...
    const nGroups = layout.numGroups

    const validErr = validateGroupLayout(players.length, ppg, advance)
    if (validErr) {
      toast({ title: 'Cannot create stage', description: validErr, variant: 'destructive' })
      return
    }
...
function ConfigForm({
  ...
  const nValidErr = validateGroupLayout(players.length, ppg, nA)
  const canSubmit = nValidErr === null
  const nLabel    = groupLayoutSummary(nLayout)
      ...
      <CardContent className="flex flex-col gap-5">
        ...
        <div className={cn(
          ...
          <p className={cn('text-xs mt-0.5', canSubmit ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400')}>
            {canSubmit
              ? <>{nG} group{nG > 1 ? 's' : ''} · top {nA} qualify per group{allowThird ? ` + ${nT} best-third` : ''} → <strong>{totalQ} total qualifiers</strong>.</>
              : <>{nValidErr}</>
            }
          </p>
          {canSubmit && players.length > 0 && (
            <p className="text-xs mt-1 text-green-600 dark:text-green-500">
              ℹ {nLabel} — players snake-seeded across groups.
            </p>
          )}
        </div>
